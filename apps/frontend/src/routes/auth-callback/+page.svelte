<script>
  ...
  import { takeNext } from '$lib/nextUrl'

  let error = $state('')

  onMount(async () => {
    // Where the visitor was headed before signing in (remembered on /login),
    // or the dashboard. Read once — takeNext() clears it.
    const dest = takeNext()

    // GoTrueClient (detectSessionInUrl:true) auto-detects #access_token= from implicit OAuth flow
    const { data: { session } } = await auth.getSession()
    if (session) { goto(dest); return }

    // Wait for auth state change if session not ready yet
    const { data: { subscription } } = auth.onAuthStateChange((_event, s) => {
      if (s) { subscription.unsubscribe(); goto(dest) }
    })
