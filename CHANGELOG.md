...
### Fixed
- **Nút "Chuẩn hoá ảnh bìa" báo lỗi 400** — request gửi `Content-Type: application/json` nhưng không kèm nội dung, bị Fastify từ chối trước khi vào đến route.
...
- **Màn hình Đề thi lọc theo ngôn ngữ chọn ở thanh bên trái** — không có ô chọn ngôn ngữ riêng ở góc phải màn hình, cả app chỉ còn một chỗ đổi ngôn ngữ. Bộ lọc chủ đề (tag) cũng chỉ còn hiện tag của các đề đúng ngôn ngữ. Thẻ đề hiển thị nhãn ngôn ngữ khi đề có từ 2 ngôn ngữ trở lên. Không đề nào khớp thì màn hình nhắc đổi ngôn ngữ ở thanh bên.
- **Trang đề thi công khai có ô chọn ngôn ngữ** ở thanh điều hướng, và **mở kho đề tiếng Anh tại `/en/exams`** — trước đây chỉ có `/vi/exams`, nên các đề tiếng Anh đã đăng không xuất hiện ở bất kỳ trang công khai nào và Google không index được. Ô chọn là link thường (các trang này cố tình không tải JavaScript), chỉ liệt kê ngôn ngữ thật sự có đề: tiếng Nhật vẫn chưa mở, `/ja/exams` báo 404 cho tới khi có đề tiếng Nhật. Hai kho đề khai báo lẫn nhau bằng `hreflang`, và `sitemap-exams.xml` giờ liệt kê cả đề tiếng Anh.
...
### Changed
- **Thanh điều hướng và chân trang giống nhau trên mọi trang landing** — Thương hiệu và Liên hệ trước đây thiếu link Đề thi/Tính năng/Bảo mật/Cách dùng/FAQ ở thanh trên, và chân trang mỗi trang liệt kê một bộ link khác nhau. Nay cả ba trang dùng chung một bản (lấy theo trang chủ), nên sửa một lần là mọi trang cùng đổi.
- **Trang đề thi công khai dùng chung luôn thanh điều hướng và chân trang đó** — `/vi/exams`, `/vi/exams/topics/…`, `/vi/exams/…` trước đây có bộ khung riêng: logo dạng ảnh thay vì chữ, thanh trên chỉ 2 link, chân trang sáng màu với bộ link khác. Giờ giống hệt trang chủ, kể cả ô chọn ngôn ngữ.
- **`sitemap.xml` sinh tự động** cùng lúc với các trang landing — thêm trang mới không còn phải nhớ cập nhật tay.
- **Mở một đề bằng địa chỉ sai ngôn ngữ sẽ tự chuyển về đúng địa chỉ của nó** (ví dụ `/en/exams/<đề-tiếng-việt>` → `/vi/exams/<đề-tiếng-việt>`), thay vì hiện cùng một đề ở hai địa chỉ khác nhau. Link "Đề thi" trên trang chủ tiếng Anh và tiếng Nhật giờ trỏ sang kho đề tiếng Anh.
...
- **Tự nhận diện ngôn ngữ theo quốc gia** — khách từ Việt Nam vào `novaquiz.net` được chuyển sang bản tiếng Việt, từ Nhật sang bản tiếng Nhật, còn lại (hoặc không nhận diện được) giữ tiếng Anh. Chọn ngôn ngữ thủ công ở thanh điều hướng sẽ được ghi nhớ và luôn được ưu tiên hơn vị trí.
- **`robots.txt`, `sitemap.xml`, `llms.txt`** cho `novaquiz.net`; app (`app.novaquiz.net`) có `robots.txt` riêng chặn toàn bộ vì mọi trang đều yêu cầu đăng nhập. Sitemap khai báo đầy đủ quan hệ giữa 3 bản ngôn ngữ.
- **Dữ liệu có cấu trúc (JSON-LD)** cho landing page: thông tin tổ chức, website và toàn bộ mục Câu hỏi thường gặp — giúp Google hiển thị kết quả dạng mở rộng. Bổ sung thẻ `canonical` cho mọi trang và thẻ chia sẻ mạng xã hội cho `/brand`, `/contact` (trước đây thiếu, link chia sẻ ra Facebook/Zalo hiện trơ trụi).
- **`scripts/build-landing-i18n.js`** (dev): sinh 3 file landing từ `landing/index.src.html`. Sửa nội dung landing từ nay làm ở file `.src.html` rồi chạy lại script — có hook `PreToolUse` chặn sửa nhầm file sinh tự động và chặn commit khi output chưa được sinh lại (`.claude/settings.json`).
...
- **Logo NovaQuiz trên đầu mọi email** — email đăng nhập và toàn bộ email thông báo (kết quả bài thi, bình luận, báo lỗi, credit, form liên hệ...) giờ có logo thương hiệu đặt trong ô nền trắng bo góc bên cạnh tên NovaQuiz. Dùng PNG (Gmail/Outlook không hiển thị SVG trong email) lấy từ `landing/brand-assets/`. Phần lớn ứng dụng email chặn ảnh cho tới khi người nhận bấm "hiện ảnh", nên tên thương hiệu vẫn là chữ thật — ảnh bị chặn thì email chỉ thiếu logo chứ không mất nhận diện.
...
- **`build-push.yml` không còn build trên pull request** — trước đây mỗi PR chạy full multi-platform (amd64+arm64) build cho cả 9 service kèm ghi Buildx cache (`cache-to: type=gha,mode=max`), vừa tốn thời gian CI vừa là nguồn chính khiến Actions cache đầy nhanh; giờ chỉ build khi push/merge vào `main` hoặc tag `v*`.
