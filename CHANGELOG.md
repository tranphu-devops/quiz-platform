...
### Changed
- **Model mặc định cho tạo đề bằng AI đổi sang `google/gemini-2.5-flash-lite`** (trước đây production đang đặt `moonshotai/kimi-k2.5`). Rẻ hơn khoảng 6 lần tính trên mỗi lượt sinh đề ($0.10/$0.40 so với $0.57/$2.85 mỗi 1M token), context 1M token nên nuốt trọn tài liệu dài, và quan trọng nhất: đây là model **đọc được file PDF trực tiếp** — cả dòng Kimi lẫn Qwen đều không, nên trước đó engine `native` không thể chuyển nội dung PDF cho model. Migration tự đổi giá trị đang lưu trong DB; admin chỉnh lại được ở tab "Tạo đề bằng AI" trong `/admin`.
