...
- `src/lib/imageNormalize.js` (sharp) re-encodes any uploaded format/size to a fixed JPEG before it ever reaches S3, stepping quality down until under 500KB: `normalizeToJpeg()` → 1024×560 for `type=exam-cover|question`; `normalizeAvatarToJpeg()` → 256×256 (cover-crop, `position: 'attention'`) for `type=avatar`.
- **Cover-image backfill** (`POST /admin/normalize-cover-images`, admin-only, "Upload settings" tab on `/admin`): re-normalizes `exams.cover_image_url` for exams created before the fixed-size pipeline shipped. user-service owns S3/sharp but not the `exams` row, so it calls exam-service's internal endpoints — `GET /exams/internal/covers` (list) and `PATCH /exams/internal/:id/cover-image` (write back) — rather than writing cross-schema directly (unlike the cross-schema *reads* other services do, e.g. interaction-service joining `quiz_exams.exams`, a write from outside the owning service breaks the "service owns its schema" convention). Requires `EXAM_SERVICE_URL` in user-service env. Skips exams whose cover is already 1024×560 JPEG; per-exam failures are collected and returned, not fatal to the batch. Nginx gives this one route `proxy_read_timeout 180s` (same rationale as `/api/generator/`) since it loops fetch+sharp per exam sequentially.
...
- **PDF engine** (`admin_settings.ai_generation_pdf_engine`, default `cloudflare-ai`, allowlist `PDF_ENGINES` in `lib/llm.js`): `native` needs a model with `file` input_modality or the model silently answers "empty document" inside valid JSON; `cloudflare-ai` (free, parses to text, loses layout) works with any model; `mistral-ocr` (paid/page) is the only one reading scanned PDFs.
  **`firecrawl` is structurally different from the other three** — they are values of OpenRouter's `file-parser` plugin and never touch our code, whereas Firecrawl is a separate API we call ourselves (`lib/firecrawl.js`), converting the PDF to markdown *before* the LLM call and sending a `text` block with **no `plugins` field at all** (`PRE_EXTRACTED_PDF_ENGINES`). Two consequences worth knowing: (1) Firecrawl accepts **only a URL** — no file upload, no base64 — so `lib/docHost.js` uploads the PDF **privately** (no `public-read` ACL, unlike user-service's `lib/s3.js`) under `uploads/generator-tmp/`, hands over a 5-minute presigned URL, and deletes it in a `finally`; `storeInCache:false, maxAge:0` keeps Firecrawl from retaining the text. (2) The engine needs `FIRECRAWL_API_KEY` **and** the `AWS_*` vars; if either is missing the route **falls back to `DEFAULT_PDF_ENGINE` before deducting credits** — deliberate, since credits are spent before the background job runs and an admin misconfiguration would otherwise charge teachers for guaranteed failures. Because failure arrives as a *successful* LLM response, `generateExam()` treats "zero questions" and "all-questions-no-options" as the same `reason: 'empty_exam'`, storing the model's own `title`/`description` plus `pdf_engine`/`file_type`/`file_size_bytes` in `generation_jobs.error_detail`. Max upload `admin_settings.ai_generation_max_file_size_mb` (default 20MB); Nginx `/api/generator/` raises `client_max_body_size` to `20m`, timeouts to `180s`.
...
- **`src/params/lang.js`** gates the prefix, reading `PUBLIC_LANGS` from `src/lib/seo.js` — one list, which also drives the switcher and the hreflang set. `vi` + `en` today; `/ja` 404s until Japanese exams exist (a prefix with no content behind it renders an empty listing). Adding a language = `PUBLIC_LANGS` + a dictionary in `src/lib/i18n/public.js` + `CATALOG_PATH` in `scripts/build-landing-i18n.js`.
- **A slug resolves without a language**, so `[slug]/+page.server.js` **308s when `exam.language !== lang`** — otherwise every enabled prefix would serve the same exam at its own URL, under an `<html lang>` that lies about it. hreflang alternates go on the **catalog only** (page 1): the catalogs are language variants of one page, while an exam or a topic hub exists in exactly one language and has no translation to point at.
...
- **Chrome is the landing page's, not a second copy** — `[lang=lang]/+layout.svelte` renders `$lib/components/public/PublicChrome.svelte`, **generated** from `landing/partials/{header,footer}.html` + `chrome.css` by `scripts/build-landing-i18n.js` (as are the nav/footer strings in `$lib/i18n/chrome-strings.js`, which `publicT` merges *under* its own dict). Never hand-edit those two files; edit the partial and re-run. Two deliberate differences from the static pages, both from `csr = false`: the language `<select>` becomes plain `<a hreflang>` links (a `<select>` needs `nqSetLang()` and there is no JS to run it), built from the `langs` prop = `langSwitchLinks()` — every entry points at that language's **catalog**, not at a translation of the current page, which for an exam or a topic hub would 404; and links arrive as props so `$lib/seo.js` can make them absolute to `novaquiz.net`.
...
`landing/partials/` is what keeps the pages from drifting apart, injected at `INCLUDE:` markers: `head.html` (favicon/fonts/Umami), `base.css` (reset + brand tokens + body), `chrome.css` (nav + lang select + footer), `header.html`, `footer.html`, and `i18n.json` (shared nav/footer strings, merged **under** each page's own dict). The two stylesheets go in at the top of each page's `<style>`, in that order, so page CSS can override them. Page-only assets stay in the page — e.g. `brand.src.html` loads JetBrains Mono itself.

**The same partials also feed the app**: `chrome.css`, `header.html`, `footer.html` and `i18n.json` are compiled into `apps/frontend/src/lib/components/public/PublicChrome.svelte` + `src/lib/i18n/chrome-strings.js` for the [public SEO pages](#public-seo-pages-novaquiznetlangexams), so one edit changes both. That is why `base.css` is separate — the app already has its own reset and `:global(:root)` tokens, and `chrome.css` may therefore only use tokens **both** sides define (`--primary`, `--accent`, `--bg`, `--text`, `--muted`, `--border`).
...
> Enforced tự động bởi hook `PreToolUse` (`.claude/settings.json`): `Edit|Write` chặn sửa mọi file sinh tự động — `landing/*.html`, `sitemap.xml`, `PublicChrome.svelte`, `chrome-strings.js` (trừ `*.src.html` và `partials/`); `Bash` chặn `git commit` khi nguồn landing được stage mà output chưa sinh lại — hook tự chạy generator rồi yêu cầu `git add` lại.
...
- **One language control in the app:** the sidebar switcher (`$locale`). `/exams` filters the catalog on it (client-side, `languages` ∋ `$locale`) and has **no picker of its own** and no "all languages" opt-out — a second control could only ever disagree with the first. An empty grid there means "no exams in this language", so the empty state points at the sidebar.
