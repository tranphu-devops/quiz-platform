...
- **LLM call** (`lib/llm.js`) — plain `fetch` to OpenRouter's chat completions endpoint (no SDK dependency), `response_format: { type: 'json_schema', json_schema: { strict: true, schema } }` forces a structured `{ title, description, tags, questions[] }` result (schema mirrors the Teacher API's question shape: `content, options[{key,text}], correct_answer[] , question_type, explanation, points`). Model ids are OpenRouter slugs; the default is `google/gemini-2.5-flash-lite` (`DEFAULT_MODEL` in `lib/llm.js`, overridden by `admin_settings.ai_generation_default_model` when set) — chosen for cost per generated exam and because it lists `file` in its OpenRouter `input_modalities`, which the `native` PDF engine requires. **Check that field before changing the default**: the Moonshot (Kimi) and Qwen families do not have it, and with `engine: 'native'` that fails silently (see the PDF engine note above). The result is re-validated locally (unique option keys, `correct_answer` ⊆ option keys, `multiple` needs ≥2 correct, `order_index` assigned sequentially 0..n-1) before import — exam-service defaults `order_index` to `0` for every question if the caller omits it, so this must always be sent explicitly.
