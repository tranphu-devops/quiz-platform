...
description: >
  ...
  Indigo-violet brand palette, Inter typography, clean card surfaces with subtle
  purple-tinted borders. Landing page is light-only; quiz app supports user-toggled
...
**QuizPlatform** là nền tảng thi trực tuyến cho người dùng Việt Nam — học sinh, giáo viên và admin. Bộ nhận diện thương hiệu xây dựng trên gradient indigo-violet (`#6366f1 → #8b5cf6`), phản ánh sự hiện đại, tin cậy và học thuật.
...
Logo / brand text dùng gradient: `linear-gradient(135deg, #6366f1, #8b5cf6)` với `-webkit-background-clip: text`.

## Colors

### Brand gradient
Màu chính là cặp **Indigo `#6366f1` → Violet `#8b5cf6`**. Gradient này xuất hiện tại:
- Logo / brand text (background-clip: text)
- Nút CTA primary (background gradient)
- Avatar fallback (initials circle)
- Progress bar fill

`primary-dark` (`#4f46e5`) dùng cho hover state của nút primary.
`primary-light` (`#ede9fe`) dùng làm nền highlight (active nav link, selected option, hover background).
...
|---------|-----------------------------------------|---------------------------------------|
| Default | `0 4px 20px rgba(99,102,241,0.08)`     | `0 4px 20px rgba(0,0,0,0.4)`         |
| Hover   | `0 12px 36px rgba(99,102,241,0.18)`    | `0 12px 36px rgba(0,0,0,0.55)`       |
...
### Buttons
- **Primary**: gradient background `linear-gradient(135deg, #6366f1, #8b5cf6)`. Hover: solid `#4f46e5` + `translateY(-1px)`.
...
Border `1px solid {border}`, `border-radius: {rounded.sm}`. Focus: `border-color: {primary}` + `box-shadow: 0 0 0 3px rgba(99,102,241,0.15)`.  
...
- student: `rgba(99,102,241,0.15)` / `{primary}`
...
**Do:**
- Dùng brand gradient đồng nhất trên cả landing page và quiz app — không dùng plain `#4f46e5` solid cho brand text hay logo.
...
**Don't:**
- Đừng dùng màu `#1d4ed8` (blue-700) làm brand primary — đây là màu cũ của landing page, đã thống nhất về `#6366f1`.
