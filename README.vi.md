...
Ứng dụng tạo & thi trắc nghiệm trực tuyến, kiến trúc microservices. Một PostgreSQL dùng chung (mỗi service một schema riêng), Nginx là ingress duy nhất, auth qua GoTrue (email/mật khẩu + Google OAuth).
...
| user-service | 3002 | 4002 | Profile, upload ảnh (S3), credit, admin settings, Teacher API key |
...
| notification-service | 3007 | 4007 | Thông báo admin + hoạt động cá nhân (Email/Pushover/Telegram) |
...
> `apps/auth-service/` là prototype cũ, không gắn vào Compose/Nginx — bỏ qua.
...
- Expose thêm port trên host (4000–4007, 9999, 5432, 6379)
...
- **Tạo đề bằng AI**: teacher upload tài liệu (PDF/DOCX/text) tại `/exams/generate` — hệ thống gọi LLM (qua OpenRouter) soạn sẵn bộ câu hỏi trắc nghiệm, tự tạo đề nháp để hoàn thiện. Dùng key LLM riêng của teacher hoặc key nền tảng do admin cấu hình (trừ credit).
- **Bộ đề & Huy hiệu**: gom nhiều đề thành lộ trình; hoàn thành toàn bộ → tự động trao huy hiệu trên profile.
- **Hệ thống Credit**: mỗi đề có credit cost; trừ credit khi bắt đầu thi; admin cấu hình mức credit. Chương trình giới thiệu bạn bè: mời bạn đăng ký, cả hai nhận credit khi người được mời nâng cấp thành teacher hoặc phát sinh hành động trả phí.
...
- **Thông báo đa kênh**: nhận cảnh báo hoạt động cá nhân (chấm bài xong, có bình luận trả lời, báo lỗi được xử lý...) và cảnh báo hệ thống/admin, gửi qua Email (Resend), Pushover hoặc Telegram — mỗi người bật/tắt riêng theo từng loại sự kiện tại `/profile`.
- **Teacher API**: API key dài hạn (`/profile`) cho teacher quản lý đề thi bằng script mà không cần đăng nhập trình duyệt — xem `/api-docs`.
- **Admin System Overview**: xem tình trạng service, log, thống kê DB read-only trên `/admin`, không cần SSH.
...
- **Cache**: Redis (cache đọc best-effort cho exam/user/interaction service)
- **Storage**: Lightsail / S3-compatible object storage (upload ảnh)
- **Container**: Docker Compose + Nginx
- **Registry**: GitHub Container Registry (GHCR)
- **CI/CD**: GitHub Actions — matrix build (user / exam / submission / interaction / generator / notification / grader / frontend), auto-deploy lên server sau khi build thành công
